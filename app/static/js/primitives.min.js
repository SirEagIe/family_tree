/**
 * @preserve Basic Primitives orgDiagram v2.0.20
	...
	if (parentTreeItem.partners.length > 1 || parentTreeItem.extraPartners.length > 0) {
	    ...
	    if (points.length > 0) {
	        ...
	        buffer.addInverted(function (invertedBuffer) {
	            self._drawTopConnectors(invertedBuffer, parentHorizontalCenter, partnersConnectorOffset, points, true, hasGraphics);
	        });
	    }
	}
	...
	if (parentTreeItem.visualChildren.length > 0) {
		...
		if (!hideConnectors && points.length > 0) {
		    ...
		    this.transform.transformPoints(parentHorizontalCenter, partnersConnectorOffset,
                parentHorizontalCenter, parentTreeLevel.connectorShift,
		    ...
		    this._drawTopConnectors(buffer, parentHorizontalCenter, parentTreeLevel.connectorShift, points, hasSquared, hasGraphics);
		}
	}
...
primitives.orgdiagram.BaseController.prototype._drawTopConnectors = function (buffer, parentX, parentY, points, hasSquared, hasGraphics) {
    ...
    if (hasSquared) {
        ...
        if (len > 0) {
            this._drawAngularConnectors(buffer, parentX, parentY, curvedPoints, false);
        }
        ...
        this._drawAngularConnectors(buffer, parentX, parentY, points, true);
    }
};
...
primitives.orgdiagram.BaseController.prototype._onDefaultTemplateRender = function (event, data) {//ignore jslint
  ...
  data.element.find("[name=title]").attr({"id": itemConfig.id});
  if(itemConfig.date_of_death != 'None') {
    data.element.find("[name=description]").text(itemConfig.date_of_birthday + ' - ' + itemConfig.date_of_death);
  }
  else {
    data.element.find("[name=description]").text(itemConfig.date_of_birthday + ' - ...');
  }
};
...
primitives.famdiagram.TemplateConfig = function () {
    ...
    */
	this.itemSize = new primitives.common.Size(220, 190);
...
primitives.famdiagram.Config = function (name) {
    ...
    */
	this.groupByType = 1/*primitives.common.GroupByType.Parents*/;
...
*/
    ...
    */
	this.printPreviewPageSize = new primitives.common.Size(1612, 1792);
...
/*
    Class: primitives.famdiagram.LabelAnnotationConfig
	    Options class. Populate annotation collection with instances of this objects to draw labels between items in diagram.
        All referenced items must be referenced by annotated item. If you need to create cross chart reference then use connector annotation.
...
primitives.famdiagram.LabelAnnotationConfig = function (arg0, arg1) {
    ...
    */
    ...
    /*
	Property: fromItem
	    Reference to from item in diagram.
	See Also:
	    <primitives.famdiagram.ItemConfig.id>
    */
    this.fromItem = null;
    ...
    /*
    Property: toItems
        Collection of referenced items in diagram.
	See Also:
	    <primitives.famdiagram.ItemConfig.id>
    */
    this.toItems = [];
...
primitives.famdiagram.Controller.prototype._virtualReadTemplates = function () {
    ...
    template = new primitives.orgdiagram.Template({
        ...
        itemSize: new primitives.common.Size(100, 200)
...
primitives.orgdiagram.TemplateConfig = function () {
    ...
    */
	this.itemSize = new primitives.common.Size(110, 110);
...
*/
	...
	*/
    ...
    Default:
        new <primitives.common.Size>(612, 792);
    */
	this.printPreviewPageSize = new primitives.common.Size(11612, 11792);
};
/*
...
primitives.orgdiagram.Buffer = function (options) {
    ...
    if(options.linesPalette.length == 0) {
        ...
        this.palette = [new primitives.famdiagram.PaletteItemConfig(options.linesColor, options.linesWidth, options.linesType)];
        this.paletteLength = this.palette.length;

        this.regularIndex = 0;
    } else {
        this.palette = options.linesPalette.slice(0);
        this.paletteLength = this.palette.length;

        /* regular */
        this.palette.push(new primitives.famdiagram.PaletteItemConfig(options.linesColor, options.linesWidth, options.linesType));
        this.regularIndex = this.palette.length - 1;
    }

    /* highlight */
    this.palette.push(new primitives.famdiagram.PaletteItemConfig(options.highlightLinesColor, options.highlightLinesWidth, options.highlightLinesType));
...
primitives.orgdiagram.Buffer.prototype._getPolyline = function (polylines, index) {
    if (!polylines[index]) {
        polylines[index] = new primitives.orgdiagram.Polyline(this.palette[index]);
    }
...
primitives.orgdiagram.Buffer.prototype.getPolyline = function (connectorStyleType) {
    ...
    return this._getPolyline(this.polylines, index);
};
...
primitives.orgdiagram.Buffer.prototype.getPolylines = function () {
    var index, len,
        polyline,
        result = [];
    for (index = 0, len = this.polylines.length; index < len; index+=1) {
        polyline = this.polylines[index];
        if (polyline) {
...
primitives.orgdiagram.Buffer.prototype.addInverted = function (callbackFun) {
    var index, len,
        backupPolylines,
    ...
    for (index = 0, len = this.polylines.length; index < len; index += 1) {
        polyline = this._getPolyline(this.polylines, index);
        backupPolyline = this._getPolyline(backupPolylines, index);
...
primitives.orgdiagram.Polyline.prototype.addInverted = function (polyline) {
    var index,
...
primitives.orgdiagram.Polyline.prototype.optimizeMoveSegments = function () {
    var index, len,
        cursorIndex,
        key,
        optimizedSegments,
...
primitives.orgdiagram.Template.prototype._getDefaultItemTemplate = function () {
    ...
    }).addClass("bp-item bp-corner-all bt-item-frame item").attr({"name": "tree_item"});
